"""
Test suite for dashboard visualization components
"""
...
def dash_duo(dash_thread_server):
    ...
    with DashComposite(dash_thread_server) as dc:
        yield dc
...
def test_learning_insights_charts(dash_duo, monitor_manager):
    ...
    dash_duo.wait_for_element("#performance-prediction .js-plotly-plot")
    ...
    assert dash_duo.find_element("#performance-prediction")
...
def test_anomaly_detection_visualization(dash_duo, monitor_manager):
    ...
    history.loc[anomaly_indices, 'profit'] = -1000.0  # Anomalous profit
    ...
    # Wait for anomaly chart and alerts
    dash_duo.wait_for_element("#anomaly-detection-chart .js-plotly-plot")
    dash_duo.wait_for_element("#anomaly-alerts")
    
    # Verify chart and alerts
    chart = dash_duo.find_element("#anomaly-detection-chart")
    alerts = dash_duo.find_element("#anomaly-alerts")
    
    assert "Anomaly Score" in chart.text
