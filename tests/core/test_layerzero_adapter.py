...
from src.core.layerzero_adapter import LayerZeroAdapter, LayerZeroConfig
...
class TestLayerZeroAdapter:
    """Test suite for LayerZero adapter"""
    
    def test_initialize_adapter(self, web3_mock, bridge_config):
        """Test adapter initialization"""
        adapter = LayerZeroAdapter(bridge_config, web3_mock)
        
        assert adapter.lz_config.version == 'v2'
        assert adapter.lz_config.executor_config['gas_limit'] == 200000
        assert adapter.lz_config.uln_config['confirmations'] == 20
        assert adapter.lz_config.message_library == 'latest'
    ...
    def test_estimate_fees(self, web3_mock, bridge_config):
        ...
        assert 'executor_fee' in fees
        assert 'oracle_fee' in fees
        assert 'total' in fees
        assert fees['total'] == fees['base_fee'] + fees['executor_fee'] + fees['oracle_fee']
    ...
    def test_prepare_transfer(self, mock_contract, web3_mock, bridge_config):
        ...
        mock_contract.return_value.functions.send.return_value.build_transaction.return_value = {
    ...
    def test_monitor_liquidity(self, web3_mock, bridge_config):
        ...
        assert liquidity == float('inf')  # LayerZero is a messaging protocol
